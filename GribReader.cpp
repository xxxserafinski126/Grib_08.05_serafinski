...
void GribReader::readSection1() {
    ...
    // Czytanie niższego bajtu
    gribFile.read(reinterpret_cast<char*>(&low_byte), 1);

    // Łączenie bajtów w wartość 16-bitową, uwzględniając big-endian
...
void GribReader::readSection2() {
   ...
   /*costam = (static_cast<unsigned char>(buffer[0]) << 16) |
    ...
    // Odczyt trzech bajtów dla La2
    gribFile.read(reinterpret_cast<char*>(lat_bytes), 3);
    // Odczyt trzech bajtów dla Lo2
    ...
    bool vector_Components = resolution_And_Component_Flags & 0x20; // Bit 5
    ...
    if (latitude_value & 0x800000) {
        ...
        latitude_value = -((~latitude_value + 1) & 0xFFFFFF);  // Ujemna wartość
    }

    if (longitude_value & 0x800000) {
        longitude_value |= 0xFF000000;  
        longitude_value = -((~longitude_value + 1) & 0xFFFFFF);  // Ujemna wartość
    }
